  ...
  return (
    <span className={`inline-flex ${className}`.trim()}>
      <span className="group relative inline-flex items-center">
        ...
        {hasTooltip && (
          <span
            className="pointer-events-none absolute left-1/2 top-full z-50 mt-2 -translate-x-1/2 whitespace-nowrap rounded-md border border-[var(--border-soft)] bg-[var(--surface-4)] px-2 py-1 text-xs font-medium text-[var(--text-primary)] opacity-0 transition-opacity duration-150 group-hover:opacity-100">
            {resolvedTooltip}
          </span>
        )}
      </span>
