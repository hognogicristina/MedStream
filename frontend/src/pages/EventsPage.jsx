        ...
        <header className="console-topbar rounded-[24px] p-6 sm:p-8">
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-[#b6bec9] sm:text-base">Operational event log with filters, sort order, and paging.</p>
            </div>
          </div>
        </header>
        ...
        <section className="monitor-card rounded-[28px] p-6">
          <div className="mb-6 grid gap-3 lg:grid-cols-4">
            ...
            <div className="monitor-panel rounded-2xl p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-white">{latestEventTime ? new Date(latestEventTime).toLocaleTimeString() : "--"}</p>
              <p className="mt-2 text-sm text-[#b6bec9]">{latestEventTime ? new Date(latestEventTime).toLocaleDateString() : "Waiting for live activity"}</p>
            </div>
          </div>
          ...
          <DataTable
            ...
            renderHeader={() => (
              <div className="grid grid-cols-[1fr_0.8fr_2.4fr_1.1fr] gap-3 border-b border-[#3b424b] bg-[#1b2430] px-4 py-3 text-xs font-semibold uppercase tracking-[0.22em] text-[#879196]">
                <div>Event Type</div>
                <div>Patient</div>
                <div>Message</div>
                <div>Created</div>
            ...
            renderRow={(event) => (
              <>
                <div>
                  <span className="console-chip-success inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em]">
